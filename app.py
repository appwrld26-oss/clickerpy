...
# --- اتصال آمن بقاعدة البيانات مع معالجة الأخطاء لمنع الشاشة البيضاء ---
@st.cache_resource
def init_connection():
    try:
...
def run_query(query, params=()):
    if conn is None:
        st.error("❌ لا يوجد اتصال فعال بقاعدة البيانات.")
        return False
    try:
        cur = conn.cursor()
...
# =====================================================================
# 2. إعداد الجداول وكافة العلاقات (تعمل فقط عند توفر الاتصال)
# =====================================================================
if conn is not None:
    try:
        cur = conn.cursor()
...
if conn is None:
    st.error("❌ تعذر الاتصال بقاعدة البيانات على DigitalOcean. يرجى التحقق من حالة السيرفر أو بيانات الاتصال.")
    st.stop()
...
# 2. إدارة التحديثات الإجبارية (مع دفع الرابط والإشعار الفوري لكل التطبيقات المحملة)
elif choice == "🚀 إدارة التحديثات الإجبارية":
    st.title("🚀 إدارة التحديثات الإجبارية ونشر الرابط الفوري")
    st.warning("تحذير: عند حفظ ونشر التحديث، سيقوم النظام تلقائياً بدفع رسالة التحديث ورابط التحميل كإشعار فوري (StatusBar PUSH) لجميع التطبيقات المحملة على الهواتف لجعلهم يقومون بالتحديث فوراً.")
    ...
    with st.form("update_form"):
        ...
        if st.form_submit_button("حفظ ونشر التحديث الإجباري وإشعار البار لكل التطبيقات 🚀"):
            res_conf = run_query("""
            ...
            push_broadcast = f"PUSH:{new_msg} | رابط التحميل: {new_url}"
            res_push = run_query("UPDATE myapp.users_status SET notice_message = %s, force_update_single = TRUE", (push_broadcast,))
            
            if res_conf and res_push:
                st.success("✅ تم حفظ ونشر التحديث الإجباري وإرسال تنبيه البار لجميع المستخدمين المحملين بنجاح!")
                st.rerun()
            ...
            with c_act2:
                if st.button("🗑️ حذف الحساب نهائياً"):
                    if selected_acc == "admin":
                        ...
                        run_query("UPDATE myapp.app_permissions WHERE username = %s", (selected_acc,))
