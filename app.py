import streamlit as st import pandas as pd import psycopg2 import plotly.express as px import random import string from datetime import datetime, timedelta
=====================================================================
...
=====================================================================
@st.cache_resource def get_conn(): try: return psycopg2.connect( database="defaultdb", user="doadmin", password="1tHwqXCgn8BS6iTm942V3f7a", host="myclicker-db-rd7ky.db1.ondigitalocean.com", port="5432", sslmode="require", connect_timeout=5 ) except Exception: return None
conn = get_conn() if not conn: st.error("❌ فشل الاتصال بقاعدة البيانات على DigitalOcean. يرجى التحقق من بيانات الاتصال.") st.stop()
def query(sql, params=()): try: global conn if conn is None or conn.closed != 0: conn = get_conn() if not conn: return False cur = conn.cursor() cur.execute(sql, params) conn.commit() cur.close() return True except Exception as e: if conn: try: conn.rollback() except Exception: pass st.error(f"خطأ في تنفيذ قاعدة البيانات: {e}") return False
@st.cache_data(ttl=10) def load_users_data(): try: if conn and conn.closed == 0: return pd.read_sql("SELECT device_id, phone, status, subscription_type, expiry_date, bot_status, app_version, accepted_clicks, last_active, notice_message FROM myapp.users_status ORDER BY last_active DESC", conn) except Exception: pass return pd.DataFrame()
@st.cache_data(ttl=10) def load_subs_data(): try: if conn and conn.closed == 0: return pd.read_sql("SELECT id, code, sub_type, duration_days, is_used, used_by_device, used_at FROM myapp.subscriptions ORDER BY id DESC", conn) except Exception: pass return pd.DataFrame()
@st.cache_data(ttl=15) def load_config_data(): try: if conn and conn.closed == 0: config_rows = pd.read_sql("SELECT key, value FROM myapp.app_config", conn) return dict(zip(config_rows['key'], config_rows['value'])) except Exception: pass return {'latest_version': '7.2.0', 'update_url': '', 'force_update': 'true', 'update_message': 'يرجى التحديث'}
=====================================================================
تهيئة الجداول وتحديث الهيكل الذاتي وصلاحيات الأدمن
=====================================================================
try: if conn and conn.closed == 0: cur = conn.cursor() cur.execute("CREATE SCHEMA IF NOT EXISTS myapp;") cur.execute(""" CREATE TABLE IF NOT EXISTS myapp.app_permissions ( id SERIAL PRIMARY KEY, username VARCHAR(50) UNIQUE NOT NULL, password VARCHAR(100) NOT NULL, role_name VARCHAR(50), allowed_sections TEXT[], is_active BOOLEAN DEFAULT TRUE ); """) cur.execute("ALTER TABLE myapp.app_permissions ADD COLUMN IF NOT EXISTS is_active BOOLEAN DEFAULT TRUE;")
    all_secs = [
    ...
    conn.commit()
    cur.close()
except Exception as e: if conn: try: conn.rollback() except Exception: pass
=====================================================================
نظام المصادقة
=====================================================================
if "logged" not in st.session_state: st.session_state.logged = False
if not st.session_state.logged: st.title("🔐 تسجيل الدخول - لوحة تحكم MyClicker Pro") with st.form("login"): u = st.text_input("اسم المستخدم:") p = st.text_input("كلمة المرور:", type="password") if st.form_submit_button("تسجيل الدخول 🚀"): try: if conn and conn.closed == 0: cur = conn.cursor() cur.execute("SELECT password, allowed_sections, is_active FROM myapp.app_permissions WHERE username = %s", (u,)) res = cur.fetchone() cur.close() if res and res[2] and res[0] == p: st.session_state.logged = True st.session_state.user = u st.session_state.sections = res[1] if res[1] else [] st.rerun() else: st.error("بيانات الدخول غير صحيحة أو الحساب معطل.") else: st.error("فشل الاتصال بقاعدة البيانات.") except Exception as login_err: st.error(f"خطأ أثناء تسجيل الدخول: {login_err}") st.stop()
=====================================================================
الشريط الجانبي (Sidebar والصلاحيات)
=====================================================================
st.sidebar.markdown(f"### ⚡ MyClicker Pro\n👤 المستخدم: {st.session_state.user}") if st.sidebar.button("🔄 مسح الذاكرة المؤقتة والتحديث"): st.cache_data.clear() st.rerun()
user_allowed_sections = st.session_state.get('sections', []) if not user_allowed_sections: st.warning("⚠️ ليس لديك أي صلاحيات لعرض الأقسام. يرجى مراجعة مدير النظام.") if st.sidebar.button("🚪 تسجيل الخروج"): st.session_state.logged = False st.rerun() st.stop()
page = st.sidebar.radio("القائمة الرئيسية:", user_allowed_sections)
if st.sidebar.button("🚪 تسجيل الخروج"): st.session_state.logged = False st.rerun()
=====================================================================
الأقسام البرمجية للوحة التحكم
=====================================================================
if page == "📈 نظرة عامة وإحصائيات الإصدارات": st.title("📈 لوحة المؤشرات الحية وإحصائيات إصدارات التطبيق") df_u = load_users_data()
total_subs = len(df_u)
...
elif page == "👥 إدارة ومراقبة المستخدمين والتفعيل": st.title("👥 إدارة المستخدمين، الأجهزة، والتحكم ببيانات التفعيل والحذف") df_users = load_users_data()
if not df_users.empty:
    ...
    with st.form("edit_user_activation_form"):
        ...
        if reset_device_clicked:
            # نقوم بمسح معرف الجهاز من السجل للسماح للهاتف بالارتباط بجهاز جديد
...
elif page == "📢 مركز الإشعارات الشامل الكامل": st.title("📢 مركز الإشعارات الشامل المتقدم") st.info("💡 تحكم كامل بإرسال الإشعارات والرسائل المنبثقة وشريط الحالة الفورية للأجهزة.")
...
with tab_manage:
    ...
    with st.form("live_update_form"):
        ...
        lk = st.text_area("الكلمات المفتاحية للقبول (live_keywords):", 
                          value=conf.get('live_keywords', ''), 
                          help="افصل بين الكلمات بفاصلة (،)")
        li = st.text_area("مؤشرات الطلب (live_indicators):", 
                          value=conf.get('live_indicators', ''), 
                          help="افصل بين المؤشرات بفاصلة (،)")
        
        st.markdown("---")
        st.subheader("🌐 توجيه السيرفر الديناميكي")
        n_url = st.text_input("رابط الـ API القادم (next_api_url):", 
                              value=conf.get('next_api_url', ''), 
                              help="سيتم استخدامه لتوجيه التطبيق إلى دومين جديد تلقائياً")
        ...
        if st.form_submit_button("🚀 حفظ ونشر التحديثات الحية"):
            ...
            st.rerun()
elif page == "🚀 إدارة التحديثات الإجبارية": st.title("🚀 إدارة التحديثات الإجبارية") conf = load_config_data()
with st.form("upd_form_node"):
    ...
    if st.form_submit_button("حفظ ونشر التحديث الإجباري 🚀"):
        ...
        st.rerun()
elif page == "🎫 توليد وإدارة الأكواد": st.title("🎫 توليد وإدارة الأكواد والاشتراكات") with st.form("gen"): tp = st.selectbox("نوع الاشتراك:", ["VIP", "TRIAL"]) days = st.number_input("المدة بالأيام:", min_value=1, value=30) qty = st.number_input("الكمية المراد توليدها:", min_value=1, value=10) if st.form_submit_button("توليد الأكواد الآن 🚀"): for _ in range(qty): code = tp[:3].upper() + ''.join(random.choices(string.ascii_uppercase + string.digits, k=6)) query("INSERT INTO myapp.subscriptions (code, sub_type, duration_days, is_used) VALUES (%s, %s, %s, FALSE) ON CONFLICT DO NOTHING", (code, tp, days)) st.cache_data.clear() st.success(f"تم توليد {qty} كود اشتراك بنجاح.") st.rerun()
elif page == "🤝 قسم الشركاء (الموزعين)": st.title("🤝 لوحة الشركاء والموزعين") df_s = load_subs_data() if not df_s.empty: t1, t2 = st.tabs(["📦 الأكواد المتاحة للتوزيع", "✅ الأكواد المستخدمة من العملاء"]) t1.dataframe(df_s[df_s['is_used'] == False], use_container_width=True) t2.dataframe(df_s[df_s['is_used'] == True], use_container_width=True) else: st.info("لا توجد أكواد مسجلة حالياً.")
elif page == "📈 تحليل البيانات": st.title("📈 تحليل البيانات وأوقات الذروة للطلبات") try: if conn and conn.closed == 0: df_orders = pd.read_sql("SELECT order_time, price FROM myapp.accepted_orders LIMIT 2000", conn) if not df_orders.empty: df_orders['hour'] = pd.to_datetime(df_orders['order_time']).dt.hour fig = px.bar(df_orders.groupby('hour').size().reset_index(name='count'), x='hour', y='count', title="أوقات الذروة للطلبات المقبولة حسب الساعة") st.plotly_chart(fig, use_container_width=True) else: st.info("لا توجد سجلات طلبات كافية لعرض الرسومات البيانية.") else: st.info("لا يوجد اتصال نشط بقاعدة البيانات.") except Exception: st.info("بيانات الطلبات غير متوفرة في قاعدة البيانات حالياً.")
elif page == "🖥️ حالة السيرفر": st.title("🖥️ مراقبة حالة الخادم وقاعدة البيانات") st.success("🟢 سيرفر Node.js متصل بقاعدة بيانات PostgreSQL ويعمل بكفاءة عالية على DigitalOcean.") st.info("قاعدة البيانات: PostgreSQL | التشفير: SSL Active | بيئة العمل: Production")
elif page == "🔐 إدارة الصلاحيات والتحكم": st.title("🔐 إدارة حسابات لوحة التحكم وصلاحيات الأقسام") st.info("من هنا يمكنك إضافة مستخدمين جدد، تعديل صلاحيات وحسابات المستخدمين الحاليين، أو حذف الحسابات غير المرغوبة.")
...
elif page == "🛠️ الدعم الفني والتواصل": st.title("🛠️ الدعم الفني وقنوات التواصل") st.info("📱 واتساب الإدارة: مراسلة الدعم الفني") st.success("✈️ تليجرام الدعم الفني: @MyClicker_Support")    اضف  لوحة  LIVE UPDATE
