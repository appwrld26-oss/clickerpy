...
def run_query(query, params=None, is_select=True):
    try:
        ...
        st.error(f"❌ فشل البيانات: {e}"); return None
...
with st.sidebar:
    ...
    menu = st.radio("**وحدات التحكم:**", [
        ...
        "👥 إدارة السائقين والرقابة الثلاثية",
...
# ============================================================
# 4. تنفيذ كافة المزايا (Ultimate Omega Logic)
# ============================================================

# 4.1 مركز القيادة (تفعيل جماعي + مفتاح الاشتراك الفردي)
if menu == "📈 مركز القيادة والتحكم الجماعي":
    st.markdown("<div class='hero'><h1>📈 مركز السيطرة والقرارات الجماعية</h1><p>إدارة الأسطول بالكامل وتفعيل الأنظمة المركزية.</p></div>", unsafe_allow_html=True)
    ...
    c1.metric("إجمالي الكباتن", f"{int(res['total'])}")
    c2.metric("إجمالي الصيد 🎯", f"{int(res['clicks'] or 0)}")
    
    # مفتاح نظام الاشتراكات الفردية (Master Switch)
    ...
    st.divider()
    st.subheader("⚡ التحكم الخارق (Mass Override)")
    col_a, col_b = st.columns(2)
    if col_a.button("✅ تفعيل كافة الحسابات فوراً (30 يوم)"):
        run_query("UPDATE myapp.users_status SET status='Active', expiry_date=NOW()+interval '30 days', is_frozen=false WHERE device_id NOT LIKE 'sim_%%'", is_select=False)
        st.success("تم تفعيل كافة المستخدمين حول العالم بنجاح! 🔥")
    if col_b.button("🛡️ محسن الجدولة والاتصال"):
...
# 4.2 إدارة الأسطول (Full Audit Sheet)
elif menu == "👥 إدارة السائقين والرقابة الثلاثية":
    st.title("📂 ورقة بيانات الأسطول والرقابة")
...
# 4.3 مركز الإشعارات (Dropdown Heads-up)
    ...
    if st.button("🚀 بث الإشعار المنسدل للجميع"):
        if msg:
            ...
            st.success(f"✅ تم البث! رقم الإصدار الحالي: {new_v}")
        else: st.warning("يرجى كتابة نص الرسالة!")

# 4.4 مولد الأكواد المطور (VIP/Days Logic)
    ...
    with c1:
        ...
        if st.button("✨ إنتاج الأكواد وحفظها"):
            ...
            st.code("\n".join(new_keys)); st.success("تم الحفظ في نيون!")

# 4.5 تحديث الذكاء الحي
elif menu == "⚡ تحديث البيانات الحية (Live)":
    st.title("⚡ برمجة ذكاء البوت حياً")
    config = run_query("SELECT key, value FROM myapp.app_config")
    edited = st.data_editor(config, use_container_width=True, hide_index=True)
    if st.button("💾 تطبيق التغييرات على كافة الأجهزة"):
        ...
        st.toast("تم تحديث ذكاء كافة السائقين!")

# 4.6 التحليل 3D
elif menu == "📊 تحليل البيانات الفضائي 3D":
    st.title("🌌 التحليل الفضائي لصيد الرحلات")
    df_3d = run_query("SELECT accepted_clicks as z, phone as x, last_active as y FROM myapp.users_status WHERE accepted_clicks > 0 LIMIT 300")
