...
@st.cache_data(ttl=15) def load_config_data(): try: if conn and conn.closed == 0: config_rows = pd.read_sql("SELECT key, value FROM myapp.app_config", conn) return dict(zip(config_rows['key'], config_rows['value'])) except Exception: pass return {'latest_version': '7.1.0', 'update_url': '', 'force_update': 'true', 'update_message': 'يرجى التحديث'}
=====================================================================
